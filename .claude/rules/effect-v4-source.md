...
- **Read-only reference, not a dependency.** Never import from it, add it to a tsconfig or the pnpm workspace, or copy a file out of it — take the idiom, not the file. It sits outside every package root, so `tsc -b`, vitest, the lint plugins and prettier never see it, and `reference/**` is a global lint ignore. Keep it that way.
