...
- **`command`, not `file`.** The term could read a report an earlier step wrote, which is the predictable form for a slow tool. The diagnostics take about four seconds run four projects at a time, and a report file is one more thing to be stale in the editor and to forget in CI; the command runs once per process, which is once per `check`, once per `pnpm lint`, and once per editor session. This needed goodbones beta.9: oxlint hosts its plugins inside the linter's own process, whose per-thread AST buffers can merge into one mapping larger than a CI runner's RAM and swap, and Linux then refuses to fork it (`spawnSync /bin/sh ENOMEM`), which the first CI run of this campaign hit on every file. Since beta.9 the plugin runs every report command as it loads, while the process is still small, and a command that cannot run is one legible failure rather than one per file.
...
- `pnpm lint`, `pnpm lint:rules`, `pnpm lint:architecture` and `pnpm lint:conformance` each run the Effect diagnostics once, about four seconds, as does an editor session on loading the plugin. `check:all` already ran them; it now runs them five times.
