...
| HTTP E2E               | `modules/<f>/interface/*-http-live.integration.test.ts` and cross-module subscriber tests under `modules/<f>/event-handlers/*.integration.test.ts` | Full Layer graph against an in-process HTTP server                             | yes | End-to-end behavior including event subscribers, HTTP serialization, status codes                        |
