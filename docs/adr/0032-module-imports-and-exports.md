...
Every declaration is named `<module>Access<Queries|Commands|DomainEvents|Errors>`, held to that shape by `architecture/surface`, so a consumer's import line says which context it reaches and what kind of thing it is taking. Domain events and published errors are collected into objects rather than re-exported loose, so each grant is one name and a module adding to its own domain does not widen one. The surface may not re-export at all: a bare re-export publishes whatever the origin exports, under whatever name, and escapes the naming rule. A peer needing a type derives it from the published value; a type-only re-export is not exempted because the extractor cannot tell it from a value one. `subsetOf` (added to `@effect-server-utils/cqrs` for this) builds a group of exactly the named messages. A dispatcher over it demands only those tags' registrations, so a peer reaching for a query missing from the list needs a registration this module never published — and adding a query to `roleQueryGroup` cannot widen the grant. goodbones governs who may open the file at all.
