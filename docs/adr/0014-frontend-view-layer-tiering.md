...
- `client-tanstack-allowlist` — `@tanstack/react-query` and `@tanstack/query-core` may only be imported by `services/data-access/**`, `lib/tanstack-query/**`, and `app/providers.tsx`. Anywhere else fails CI.
