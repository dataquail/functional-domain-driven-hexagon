...
The shared kernel under `platform/ddd/` is tiered by the principle that decides what the domain may touch: **the domain may depend on shared _types/contracts_, but not acquire and invoke shared _services_.** `platform/ddd/contracts/` holds `DomainEvent`, the `SpanAttributesExtractor` type, and `PersistenceUnavailable` — types the domain merely references; `domain-isolation` allows this folder wholesale. `platform/ddd/ports/` holds `UnitOfWork` and the event buses — services the application ring `yield*`s. The command and query buses are the same tier conceptually but live in `@org/cqrs` (ADR-0006), which is why `domain-isolation` does not allowlist that package: the domain must not name a bus, and keeping the allowlist silent on it is what enforces that. It holds no cross-module ACL services: those are consumer-owned and live in each module's `domain/ports/acl/` (ADR-0022). A companion rule `ddd-contracts-no-ports` forbids `contracts/` from importing `ports/`, so the contracts tier stays transitively domain-safe.
