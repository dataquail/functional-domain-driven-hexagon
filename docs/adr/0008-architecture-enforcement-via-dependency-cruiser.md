...
- `commands-isolation`: commands may import only their own module's `domain/` and sibling `commands/`, `@effect-server-utils/cqrs` (the vocabulary a message is declared in, ADR-0006), `@effect-server-utils/unit-of-work` (`withUnitOfWork` and the `PersistenceUnavailable` a command's error channel names), and the platform DDD kernel (the command/query/event buses). No infrastructure, interface, or queries. No `@org/contracts` — a command's failure channel names domain errors and the endpoint maps them to wire errors (ADR-0004) — and no `@org/database`. **Foreign module barrels are not importable directly** — a call into another module goes through the consumer's `infrastructure/acl/` adapter (ADR-0022). Test files excluded.
- `queries-isolation`: queries may import their own module's `domain/` (IDs, value objects) and sibling `queries/`, `@effect-server-utils/cqrs`, `@effect-server-utils/unit-of-work`, the platform kernel, and `@org/database` for direct SQL projection. They may NOT import commands, infrastructure, interface, `@org/contracts`, or foreign barrels. Test files excluded.
...
The shared kernel that was under `platform/ddd/` now lives in the CQRS package (ADR-0006/0007), which changes how the same principle is enforced: **the domain may depend on shared _types/contracts_, but not acquire and invoke shared _services_.** A package barrel cannot express that tiering — it exports the event factory and the buses alike. An earlier attempt allowlisted the two domain-safe modules by their resolved file paths inside the package; it worked, but it coupled the rule to another package's internal file layout, and that package generates its published surface from every top-level file, so a rename there would silently change what the domain could import. Instead the tiering is a **folder**: those modules are re-exported under this application's vocabulary from `platform/ddd/contracts/`, `domain-isolation` admits that folder, and the domain does not name the library at all. The bus alias sits one level up, at `platform/ddd/event-bus.ts`, so a `domain/ports/` port structurally cannot declare a bus in its requirement channel — the exclusion ADR-0006's per-module dispatch surfaces depend on. Cross-module ACL services are not in the package at all: those are consumer-owned and live in each module's `domain/ports/acl/` (ADR-0022). `platform/ddd/` holds the specification kit and the two contracts-tier re-exports the domain reads, plus `event-bus.ts` — deliberately outside `contracts/`, because that folder _is_ the allowlist.
...
**Moving a package from the workspace to npm relocates its rules without warning.** The isolation rules scope their `to` clause to `^packages/`, which is what lets a single allowlist enumerate sibling folders. An installed dependency resolves under `node_modules/` instead, so it stops matching that clause altogether: every allowlist entry naming it becomes dead weight, and — the part that bites — every _prohibition_ implied by leaving it out becomes unenforced. Enforcement moves to the `*-no-external-beyond-effect` rules, which match on `dependencyTypes: npm`. When the CQRS and authorization libraries were extracted and published, that is the migration their rules underwent: the allowlist entries were deleted and the external-dependency rules widened to admit them by package name, deliberately and in one place. The rule to carry forward is that a package's _tier_ determines which family of rules governs it, and changing the tier means rewriting the rules in the other family — not editing the paths in place.
