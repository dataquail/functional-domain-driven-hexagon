...
- `commands-isolation`: commands may import only their own module's `domain/` and sibling `commands/`, `@org/cqrs` (the vocabulary a message is declared in, ADR-0006), and the platform DDD kernel (the command/query/event buses and the unit of work). No infrastructure, interface, or queries. No `@org/contracts` — a command's failure channel names domain errors and the endpoint maps them to wire errors (ADR-0004) — and no `@org/database`. **Foreign module barrels are not importable directly** — a call into another module goes through the consumer's `infrastructure/acl/` adapter (ADR-0022). Test files excluded.
- `queries-isolation`: queries may import their own module's `domain/` (IDs, value objects) and sibling `queries/`, `@org/cqrs`, the platform kernel, and `@org/database` for direct SQL projection. They may NOT import commands, infrastructure, interface, `@org/contracts`, or foreign barrels. Test files excluded.
