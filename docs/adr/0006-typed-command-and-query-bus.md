...
The table is **erased** — tag to dispatch function, no signature. It does not need one: what checks a module's handlers is that module's own registration against the same definitions the caller dispatches. Two consequences follow. A tag claimed by two modules is a wiring bug that would let the wrong module answer a message; the merge rejects it at composition time, which is the only guard, so it runs once at boot. And a tag missing from the table is a dispatch-time defect rather than a compile error.
...
The package owns the whole pattern, not half of it: declaring and handling messages (`Command.make`, `Command.group`, `Command.handlersOf`), a module's own dispatch surface (`Command.dispatcher`), and the application-wide bus — its Tag, its shape, and the routing table that folds per-module dispatchers into it. Keeping the bus out would leave every consumer to reinvent the tag router, the erased table, and the duplicate-tag guard, which is the difference between a CQRS library and an RPC wrapper.
...
- A missing tag or a duplicate tag is caught at composition time or first dispatch, not by the type checker. This is the price of the erased routing table.
- The buses stay thin. A behavior wanted across every dispatch is added once inside the bus without touching call sites.
