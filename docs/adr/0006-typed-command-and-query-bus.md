...
### Implemented on RPC, in a standalone package (since published)
...
This began as a standalone workspace package for two reasons. It keeps the abstraction honest: the package exposes command/query vocabulary only and leaks no RPC types, which is asserted by tests and by architecture rules rather than left to discipline. And the Effect ecosystem has no full CQRS library, so it was staged for eventual extraction and publication.

That extraction has since happened. The library now lives in its own repository and is consumed here as `@effect-server-utils/cqrs`, pinned to an exact beta. Three of the architecture rules written for it were retired rather than ported, because they governed the inside of a package this repository no longer contains: the two that fenced the rpc transport to one file, and the one that kept `internal/` private. The last of those is not a loss of enforcement — the published `exports` map declares no `./internal` entry, so the import fails to resolve at all, which is stronger than a lint rule and cannot be waived. The rules that survive are the ones about how _this_ application consumes the library: nothing here may name rpc, and the bus factories stay at a composition root.

The package owns the whole pattern, not half of it: declaring and handling messages, a module's own dispatch surface, the application-wide bus with its routing table, and — because a CQRS library that stops at the two request/response buses leaves consumers to reinvent the fan-out half — the domain-event bus, process managers, and the middleware seam. Keeping any of it out would leave every consumer to rebuild it, which is the difference between a CQRS library and an RPC wrapper. The unit-of-work boundary the event bus once hung off has since moved to a sibling package; what the bus keeps is an optional deferral seam that names no transaction (ADR-0007 covers the event and unit-of-work half).
