...
- **`Context.Service` `make`/`static layer`** pattern applied uniformly (not just where compilation forced it). — **DONE / assessed: no change warranted.** Surveying all 45 `Context.Service` definitions: every one that is _not_ a `static layer` is deliberately so — it is either (a) a **port** whose `Live` is a separate `*Live` Layer (the hexagonal ports/lives separation, ADR-0007 — a `static layer` on the port would leak the Live into the domain), (b) a **parameterized factory** (`(config) => Layer.effect(Tag, make(config))`, e.g. `Database.layer`, `TokenCipher.layer`, `makePolicyRegistry`) that a no-arg `static layer` cannot express, or (c) a **multi-service test Layer** built with `Layer.effectContext`. The true singletons that _can_ use `static layer` (`CookieCodec`, `EnvVars`, `OidcClient`) already do. Forcing `static layer` everywhere is impossible for (b)/(c) and gold-plating churn for the rest — the Speculative-Generality / Divergent-Change guard-rail applies. The idiom is already uniform _within each shape category_.
- **`effect/unstable/observability` (Otlp)** replaces `@effect/opentelemetry/NodeSdk`; drop the `@effect/opentelemetry` dep. Verify traces still land in Jaeger via `pnpm bootstrap`. — **DONE** (server; `OtlpTracer.layer` + `OtlpSerialization.layerJson` + `FetchHttpClient.layer`). Browser `WebSdk` port deferred as noted.
- **`use` accessors** only where they read more clearly than `yield*` (the services guide prefers `yield*` for dependency legibility — don't over-apply). — **assessed: no change.** The codebase consistently uses `yield*` for service access, which the services guide prefers for dependency legibility; there is no cluster of call sites where `use` reads better, so applying it would be over-application against the guide's own advice.
