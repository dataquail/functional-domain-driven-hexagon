...
- Exception: the required header in `*.endpoint.test.ts` parity tokens (see Endpoint test naming above).
...
| `pnpm check:effect`                                    | `@effect/language-service` diagnostics across all projects; fails on any finding (see below)    |
