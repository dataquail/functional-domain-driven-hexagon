...
# Leave 8080 free for the dev container's Host-rewriting proxy — Zitadel takes
# its user-agent cookie domain from the request Host, and GitHub's forwarder
# rewrites that to `localhost`, so the cookie is scoped to a host the browser
# is not on and gets dropped. See docker-compose.yml.
set_env ZITADEL_HOST_PORT "18080"
