...
export const dispatcher = <G extends AnyGroup>(
  ...
  Message.dispatcher(queryGroup, [
    ...
    Middleware.span({ spanPrefix: "query", attributes: options.spanAttributes as never }),
    ...(options.middleware ?? []),
  ]);

/**
 * Whether a value is a query definition. Lets a host reflect over its own
 ...
 */
export const is = (u: unknown): u is Any => Message.isMessage("query", u);

/** Whether a value is a query group — the group counterpart of `is`. */
export const isGroup = (u: unknown): u is AnyGroup => Message.isGroup("query", u);

/**
 * Reports any channel of any query in the group that cannot survive a round-trip
...
export const checkSerializable = (
  ...
  Serializable.check(queryGroup.messages);
