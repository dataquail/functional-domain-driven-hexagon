...
import * as Roles from "./roles.aggregate.js";
...
describe("Roles.empty", () => {
  it("constructs an aggregate with no roles", () => {
    const aggregate = Roles.empty(userId);
    deepStrictEqual(aggregate.userId, userId);
    deepStrictEqual([...aggregate.roles], []);
  });
});

describe("Roles.hasRole", () => {
  it("returns false on an empty aggregate", () => {
    deepStrictEqual(Roles.hasRole(Roles.empty(userId), "super_admin"), false);
  });

  it("returns true after the role is granted", () => {
    const result = Roles.grant(Roles.empty(userId), "super_admin");
    if (Either.isLeft(result)) throw new Error("expected Right");
    deepStrictEqual(Roles.hasRole(result.right.roles, "super_admin"), true);
  });
});
...
describe("Roles.grant", () => {
  ...
  it("fails AlreadyHasRole when the role is already held", () => {
    const first = Roles.grant(Roles.empty(userId), "super_admin");
    if (Either.isLeft(first)) throw new Error("expected Right");
    const second = Roles.grant(first.right.roles, "super_admin");
...
describe("Roles.revoke", () => {
  it("removes the role and emits RoleRevoked", () => {
    const granted = Roles.grant(Roles.empty(userId), "super_admin");
    if (Either.isLeft(granted)) throw new Error("expected Right");
    const result = Roles.revoke(granted.right.roles, "super_admin");
    if (Either.isLeft(result)) throw new Error("expected Right");
  ...
  it("fails DoesNotHaveRole when the role isn't held", () => {
    const result = Roles.revoke(Roles.empty(userId), "super_admin");
