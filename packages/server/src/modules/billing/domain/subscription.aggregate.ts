...
export const applyStatus = (sub: Subscription, input: ApplyStatusInput): Result => {
  const updated = Subscription.make({
    id: sub.id,
...
// status transition.
export const cancel = (sub: Subscription, now: DateTime.Utc): Result => {
  const updated = Subscription.make({
    id: sub.id,
    organizationId: sub.organizationId,
    stripeCustomerId: sub.stripeCustomerId,
