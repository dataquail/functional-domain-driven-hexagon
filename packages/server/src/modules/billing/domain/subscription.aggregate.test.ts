...
const periodEnd = DateTime.unsafeMake(new Date("2025-02-15T00:00:00Z"));

const fresh = (status = "active"): Subscription.Subscription =>
  Subscription.create({
    id: subscriptionId,
    organizationId,
...
describe("Subscription.create", () => {
  ...
  it("emits exactly one SubscriptionStarted event with the gateway ids and status", () => {
    const { events } = Subscription.create({
      id: subscriptionId,
      organizationId,
      stripeCustomerId: "cus_abc",
      stripeSubscriptionId: "sub_xyz",
...
describe("Subscription.applyStatus", () => {
  it("replaces status + currentPeriodEnd + updatedAt and preserves identity columns", () => {
    const sub = fresh("trialing");
    const newPeriodEnd = DateTime.unsafeMake(new Date("2025-03-01T00:00:00Z"));
    const { events, subscription } = Subscription.applyStatus(sub, {
      status: "active",
...
describe("Subscription.cancel", () => {
  it("sets status to 'canceled', preserves currentPeriodEnd, refreshes updatedAt", () => {
    const sub = fresh("active");
    const { events, subscription } = Subscription.cancel(sub, later);
...
describe("Subscription aggregate purity", () => {
  it("applyStatus/cancel do not mutate the input subscription", () => {
    const sub = fresh("active");
    Subscription.applyStatus(sub, { status: "past_due", currentPeriodEnd: null, now: later });
    Subscription.cancel(sub, later);
