...
import { SessionRootOps } from "./session.root.js";
...
describe("SessionRootOps.touch", () => {
  const seed = SessionRootOps.create({
    id: sessionId,
    userId,
    subject: "s",
    now,
  ...
  it("clamps expiresAt to absoluteExpiresAt when the new window would exceed the cap", () => {
    ...
    const touched = SessionRootOps.touch({ session: seed, now: nearCap, ttlSeconds: 3600 });
