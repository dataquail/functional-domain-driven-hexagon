...
export const listTokensEndpoint = Effect.fn("AuthLive.tokens.list")(function* (
  _request: EndpointRequest<typeof AuthContract.TokensGroup, "list">,
) {
  const currentUser = yield* CurrentUser;
  const queryBus = yield* QueryBus;
  const tokens = yield* queryBus.execute(ListMyApiTokensQuery, { userId: currentUser.userId });
  return tokens.map(
    (t) =>
