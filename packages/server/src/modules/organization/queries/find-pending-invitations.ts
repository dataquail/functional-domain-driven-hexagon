import * as DateTime from "effect/DateTime";
import * as Effect from "effect/Effect";

import * as Invitation from "@/modules/organization/domain/invitation.aggregate.js";
import { InvitationRepository } from "@/modules/organization/domain/ports/repositories/invitation-repository.js";
import {
  type FindPendingInvitationsOutput,
  type FindPendingInvitationsQuery,
} from "@/modules/organization/queries/find-pending-invitations-query.js";

export const findPendingInvitations = (
  query: FindPendingInvitationsQuery,
): FindPendingInvitationsOutput =>
  Effect.gen(function* () {
    const repo = yield* InvitationRepository;
    const now = yield* DateTime.now;
    const all = yield* repo.findByOrganizationId(query.organizationId);
    // Only open invitations belong on the pending list; accepted invitees
    // are members and revoked ones are gone. Status (pending/expired) is
    // derived against `now` so the UI can offer resend on lapsed invites.
    return all.filter(Invitation.isOpen).map((invitation) => ({
      invitationId: invitation.id,
      inviteeEmail: invitation.inviteeEmail,
      status: Invitation.statusAt(invitation, now),
      expiresAt: invitation.expiresAt,
      createdAt: invitation.createdAt,
    }));
  });
