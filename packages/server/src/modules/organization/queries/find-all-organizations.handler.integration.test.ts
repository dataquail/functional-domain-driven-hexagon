...
suite("findAllOrganizations (integration)", () => {
  ...
  it.effect("hides soft-deleted orgs by default", () =>
    Effect.gen(function* () {
      ...
      const result = yield* findAllOrganizations({ page: 1, pageSize: 10, includeDeleted: false });
  ...
  it.effect("returns tombstoned rows when includeDeleted is true", () =>
    Effect.gen(function* () {
      ...
      const result = yield* findAllOrganizations({ page: 1, pageSize: 10, includeDeleted: true });
