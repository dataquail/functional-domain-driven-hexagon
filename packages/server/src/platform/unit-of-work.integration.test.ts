...
import { Event, EventBus, makeEventBus } from "@effect-server-utils/cqrs";
...
// transaction. Writes go through the client, which resolves the ambient
// transaction connection per statement (the same mechanism real repositories use).
...
const insert = (sql: Statement.Constructor, id: string, email: string) =>
  sql`
    INSERT INTO "user".users (id, email, created_at, updated_at)
    VALUES (${id}, ${email}, now(), now())
  `.pipe(Database.exec);

const seeded = (sql: Statement.Constructor) =>
  sql`SELECT * FROM "user".users WHERE id IN (${outerId}, ${innerId})`.pipe(
    Database.rows(RowSchemas.UserRow),
  );

const flushed = (sql: Statement.Constructor) =>
  sql`SELECT * FROM "user".users WHERE id IN (${outerId}, ${markerId})`.pipe(
    Database.rows(RowSchemas.UserRow),
  );
...
suite("UnitOfWork re-entrancy (integration)", () => {
  ...
  it.effect("a nested run commits together with the outer transaction", () =>
    Effect.gen(function* () {
      ...
      yield* uow.run(
        Effect.gen(function* () {
          yield* insert(sql, outerId, "outer@example.com");
          // Nested run: must join the outer tx (no second connection).
          yield* uow.run(insert(sql, innerId, "inner@example.com"));
        }),
      );
      const rows = yield* seeded(sql);
  ...
  // while the outer transaction goes on to commit.
      ...
      yield* uow.run(
        Effect.gen(function* () {
          ...
          const innerExit = yield* Effect.exit(
            uow.run(
              Effect.gen(function* () {
                yield* insert(sql, innerId, "inner@example.com");
  ...
  it.effect("drains a buffered handler after the producer commits", () =>
    Effect.gen(function* () {
      ...
      yield* bus.subscribeAfterCommit(PostCommitTestEvent, () =>
        insert(sql, markerId, "marker@example.com").pipe(Effect.orDie),
      );
