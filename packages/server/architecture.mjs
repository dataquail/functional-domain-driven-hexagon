...
export const serverTree = {
  // The server's own root. One node rather than four siblings, so the taxonomy
  // has a single root, the naming convention is stated once, and a folder nobody
  // declared — test-utils/ was one until this node existed — is a violation
  // rather than a gap.
  "~/server/src/": {
    message:
    ...
    children: {
      ...
      // test-utils/ is the third composition root: it builds the whole application
      // from the same layers server.ts does, with billing's gateway faked. It
      // therefore names modules by their barrels and the Lives that back them —
      // the one place outside a composition root allowed to, because it IS one.
      "test-utils/": {
        message:
          "test-utils/ holds the server test harness: the composed test runtime, the fakes it swaps in, and the database helpers. A file here is a harness, not a feature — feature code belongs in a module.",
        imports: {
          reset: true,
          message:
            "The test harness composes the application, so it names module barrels, the platform kernel and the runtime it runs on. It must not reach past a module's barrel into its internals — the harness is a consumer of the published surface like any other.",
          external: [
            "effect",
            "vitest",
