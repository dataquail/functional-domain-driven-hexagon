...
`partial` says *"do not enumerate this folder's own file list"*. The node compiles no layout
...
[taxonomy root](/oxlint-architecture-rules/manifest/structure/). A file sitting directly in
...
In practice that means `partial` fits a folder whose *subfolders* are what you are
