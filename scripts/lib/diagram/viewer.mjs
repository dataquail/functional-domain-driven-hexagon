const labelOf = (entry) => {
  if (entry.kind === "server-usecase" || entry.kind === "server-hexagon") return entry.label;
  return entry.slug
...
const button = (entry) => `<li><button data-slug="${entry.slug}">${labelOf(entry)}</button></li>`;

const section = (title, entries) =>
  entries.length === 0
    ? ""
    : `<section><h2>${title}</h2><ul>${entries.map(button).join("")}</ul></section>`;

const navigation = (slugs) => {
  const of = (kind) => slugs.filter((entry) => entry.kind === kind);
  const useCases = of("server-usecase");
  const modules = of("server-hexagon");

  const drilldowns = modules
    .map((module) => {
      const mine = useCases.filter((entry) => entry.group === module.label);
      if (mine.length === 0) return "";
      return `<details><summary>${module.label} — ${mine.length} use cases</summary><ul>${mine
        .map(button)
        .join("")}</ul></details>`;
    })
    .join("");
  ...
  return [
    section("Modules — the map", modules),
    drilldowns === "" ? "" : `<h2>Use cases</h2>${drilldowns}`,
    section("Across modules", [...of("server-modules"), ...of("packages")]),
    section("Module shape", of("server-module")),
    section("Web", [...of("web-overview"), ...of("web-feature")]),
  ].join("");
};

export const page = (slugs) => `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<main>
  <header>
    ...
    <span class="tools">
      ...
      <button id="reload" type="button" title="redraw from the current source">redraw</button>
    </span>
  </header>
  <div class="stage" id="stage"></div>
  <footer>drawn on request from the current source &middot; amber dashed = an adapter implementing a port &middot; red = an import the rules forbid</footer>
</main>
...
<script>
  ...
  const show = async (next, keepZoom) => {
    slug = next;
    ...
    crumb.innerHTML = module
      ...
      : "";
    if (module) {
      document.getElementById("up").addEventListener("click", () => show("server-hexagon-" + module[1]));
    }

    try {
      const response = await fetch(slug + ".mmd?t=" + Date.now());
      if (!response.ok) throw new Error(await response.text());
      serial += 1;
      const { svg, bindFunctions } = await mermaid.render("render" + serial, await response.text());
      stage.innerHTML = svg;
  ...
  document.getElementById("reload").addEventListener("click", () => show(slug, true));
